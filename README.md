...
### Mahalaga
Ang SQLite file na naka-store sa normal na Render worker filesystem ay hindi garantisadong mananatili sa bawat redeploy/replacement — sa **free tier**, mabubura ang `bot.db` (kasama ang listahan ng users) sa tuwing mag-re-deploy o mag-restart ang service. Kung sa hinaharap gusto mong maging permanente ang data, kailangan mong mag-upgrade sa paid plan (Starter pataas) para magamit ang Persistent Disks, o mag-migrate sa external na database tulad ng PostgreSQL.
